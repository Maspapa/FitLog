...
FitLog 是一个低负担的个人健身打卡工具，只记录运动、体重和腰围，用长期趋势和 AI 周报帮助用户调整下一周。

## 功能

- 每日体重、腰围和运动记录；组数、次数、负重、时长选填。
- 不再填写饮食、恢复、主观状态或强度。旧字段仅保留用于历史数据和备份兼容，编辑旧记录不清空它们，AI 周报不读取它们。
- 7日平均体重、最近腰围、本周训练量和连续打卡。
- 最近30次体重趋势与14天记录本。
- DeepSeek AI 周报，输出具体证据和一个可执行的下周重点。
- 周一、周三、周五训练计划，页面内直接播放 Lyfta 动态示范并保留详情入口。
- 动作卡片展示今天之前最近一次同名运动的数据；独立加号沿用历史数值加入今天，无历史时使用计划默认值。点击动作区域展开指导，已加入的动作不可重复添加。
