      ...
      <article className="plan-day">
        ...
        {PHASES.map((phase, phaseIndex) => {
          ...
          return <section className={`plan-phase phase-${phase}`} key={phase}>
            ...
            <div className="exercise-list">{items.map((item, index) => {
              const open = openId === item.id;
              return <article className={`exercise-card ${open ? "open" : ""}`} key={item.id}>
                <button className="exercise-summary" type="button" aria-expanded={open} onClick={() => setOpenId(open ? null : item.id)}>
                ...
                {open && <div className="exercise-detail">
                  ...
                  <div className="detail-copy">
                    ...
                    {item.guideUrl && <a className="video-link" href={item.guideUrl} target="_blank" rel="noreferrer">查看专业图文 / 视频示范 <span>↗</span></a>}
                  </div>
                </div>}
              </article>;
            })}</div>
